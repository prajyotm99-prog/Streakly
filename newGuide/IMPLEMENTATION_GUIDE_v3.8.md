...
### Native Android (Java)
Place in `android/app/src/main/java/com/streakly/app/`:
...
cp android/ExactAlarmPlugin.java android/app/src/main/java/com/streakly/app/
cp android/ExactAlarmScheduler.java android/app/src/main/java/com/streakly/app/
cp android/ExactAlarmReceiver.java android/app/src/main/java/com/streakly/app/
```
...
<receiver 
    ...
    <intent-filter>
        <action android:name="com.streakly.app.EXACT_ALARM" />
    </intent-filter>
    <intent-filter>
...
```java
import com.streakly.app.ExactAlarmPlugin;
...
2. Force stop app: `Settings → Apps → Streakly → Force Stop`
...
```bash
adb shell dumpsys package com.streakly.app | grep ExactAlarmReceiver
```
...
**Fix:** Ask user to disable battery optimization for Streakly:
```
Settings → Apps → Streakly → Battery → Unrestricted
```
...
```
Settings → Apps → Streakly → Notifications → Task Reminders
```
